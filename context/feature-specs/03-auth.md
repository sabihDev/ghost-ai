Clerk is already installed and connected. Wire into Next.js app: provider, auth pages, redirects, route protection, and user menu.

## Design

Use Clerk's `dark` theme from `@clerk/ui/themes` as the base.

Override Clerk appearance variables using the app's existing CSS variables. Do not hardcore colors.
...
Wrap the root layout `ClerkProvider` using Clerk's `dark` theme.
...
- all routes are protected excepted public auth paths
